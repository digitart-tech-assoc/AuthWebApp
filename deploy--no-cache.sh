...
# AuthWebApp デプロイスクリプト（キャッシュ無し版）
# 使用方法: ./deploy--no-cache.sh [version]
# 例: ./deploy--no-cache.sh v0.1.0
#
# このスクリプトはDocker キャッシュを無視してイメージをビルドします。
# すべての層を新規にビルドするため、通常のdeploy.shより時間がかかります。
#
# 環境変数:
...
#
# 例: SKIP_DB_INIT=false ./deploy--no-cache.sh v0.1.0  # DB初期化を実行
#    ./deploy--no-cache.sh v0.1.0                      # DB初期化をスキップ（デフォルト）
...
echo "=========================================="
echo "Deploying AuthWebApp (NO CACHE) - Version: $VERSION"
echo "=========================================="
...
# 1. Docker イメージをビルド（キャッシュ無し）
echo ""
echo "[1/6] Building Docker images (NO CACHE)..."
echo "  - Building frontend with NEXT_PUBLIC_* build args..."
docker build --no-cache -t authwebapp-frontend:$VERSION -t authwebapp-frontend:latest \
...
docker build --no-cache -t authwebapp-backend:$VERSION -t authwebapp-backend:latest -f backend/Dockerfile backend/

echo "  - Building discord-bot..."
docker build --no-cache -t authwebapp-discord-bot:$VERSION -t authwebapp-discord-bot:latest -f discord-bot/Dockerfile discord-bot/
...
echo ""
echo "Build mode: NO CACHE (all layers rebuilt)"
echo ""
echo "Next steps:"
